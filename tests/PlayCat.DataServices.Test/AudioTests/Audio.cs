...
namespace PlayCat.DataServices.Test.AudioTests
{
    public class Audio : BaseTest
    {
        ...
        public void ShouldReturnAudios(int count, int skip, int take, int actual)
        {
            SqlLiteDatabaseTest(options =>
            {
                ...
                using (var context = new PlayCatDbContext(options))
                {
                    ...
                    Playlist playlist = context.CreatePlaylist(true, userId, null, 0);
        ...
        public void ShouldFailRemoveFromPlaylistNotOwner()
        {
            SqlLiteDatabaseTest(options =>
            {
                ...
                using (var context = new PlayCatDbContext(options))
                {
                    ...
                    Guid userId = GetUserId(context);
                    Playlist generalPlaylist = context.CreatePlaylist(true, userId, null, 0);

                    Playlist playlist = context.CreatePlaylist(false, userId, "Rock", 0);
        ...
        public Audio(ITestOutputHelper output) : base(output)
        {
        }
    }
}
